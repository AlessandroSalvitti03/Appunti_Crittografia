...
### **3. Le 4 Tematiche Chiave** 
La gestione dei database distribuiti deve affrontare quattro problematiche principali:
...
### **4. Frammentazione dei Dati** 
Per distribuire un database, le tabelle (relazioni) vengono scomposte in frammenti. Ogni frammentazione deve garantire due proprietà assolute: la **Completezza** (tutti i dati originali devono essere presenti nei frammenti) e la **Ricostruibilità** (deve essere possibile ricreare la tabella originale).
...
- **Corrispondenza tra frammenti e file fisici:** Ogni frammento in cui viene suddivisa una tabella corrisponde a un diverso file fisico e viene allocato su un server specifico.

- **Dati virtuali vs Dati reali:** A seguito del mapping, la tabella (o relazione) originale esiste solo in una forma "virtuale" (assimilabile a una vista), mentre i frammenti distribuiti rappresentano i dati realmente memorizzati.

- **Schema di Allocazione:** È il componente che descrive il mapping delle relazioni complete o dei frammenti verso i server che li ospitano. Questo schema è fondamentale perché permette il passaggio dalla descrizione logica dei dati alla loro descrizione fisica.

Il mapping definito dallo schema di allocazione può essere realizzato in due modi:

1. **Non ridondante:** Ogni frammento o relazione viene mappato e allocato su un singolo server.
2. **Ridondante:** Un frammento o una relazione viene mappato e allocato su più di un server contemporaneamente (creando delle repliche).
### **6. Allocazione e Livelli di Trasparenza**
...
### 8. Classificazione delle Transazioni** 
Le operazioni in un database distribuito si dividono in base alla loro complessità:
